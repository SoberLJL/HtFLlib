...
from flcore.clients.clientmgp import clientMGP
...
class FedMGP(Server):
    ...
    def receive_protos(self):
        ...
        all_class_protos = defaultdict(list)

        for client in self.selected_clients:
            self.uploaded_ids.append(client.id)
            protos = load_item(client.role, 'protos', client.save_folder_name)
            for k in protos.keys():
                for (sub_proto, count) in protos[k]:
                    self.uploaded_protos.append((sub_proto, k))
                    all_class_protos[k].append((sub_proto, count))

        # Confidence-weighted aggregation for gap computation
        avg_protos = {}
        for k, items in all_class_protos.items():
            ps = torch.stack([p for p, _ in items])
            ws = torch.tensor([w for _, w in items], dtype=torch.float32, device=ps.device)
            ws = ws / ws.sum()
            avg_protos[k] = (ps * ws.unsqueeze(1)).sum(0)

        # Per-class minimum distance
        self.gap = torch.ones(self.num_classes, device=self.device) * 1e9
    ...
    def update_TGP(self):
        ...
        for e in range(self.server_epochs):
            ...
            for proto, y in proto_loader:
                ...
                # Per-class adaptive margin instead of global uniform margin
                per_class_margin = torch.clamp(self.gap, max=self.margin_threthold)
                margin_matrix = one_hot * per_class_margin.unsqueeze(0)
                dist = dist + margin_matrix
